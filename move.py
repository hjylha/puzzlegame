...
# class Move:
#    def __init__(self, start_pos, piece_type, direction):
#        self.start_pos = start_pos
#        self.piece_type = piece_type
#        self.direction = direction

# translating moves from number into piece_id,direction pairs ????
...
def make_move(move, pos):
    ...
    # bizarre directions don't matter, hopefully
        ...
        #return pos
    return Positions(pos.stepnum + 1, new_pos)
...
def combine_lists(pos_list1, pos_list2):
    if len(pos_list2) == 1:
        return pos_list1
    pos_list = pos_list1.copy()
    for i in range(len(pos_list2)-1):
        pos_list.append(pos_list2[i+1])
